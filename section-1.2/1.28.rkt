...
; 1 modulo n. It is possible to prove that if such a nontrivial square root of 1
...
; is why the Miller-Rabin test cannot be fooled.) Modify the expmod procedure to
...
(define (non-trivial-sqrt? a n)
  (cond ((and (not (or (= a 1) (= a (- n 1))))
             (= 1 (remainder (square a) n))) #t)
        (else #f)))

(define (checked-square a n)
  (if (non-trivial-sqrt? a n)
      0
      (square a)))
...
(define (miller-rabin-test n times)
    (cond ((= times 0) #t)
            ((prime? n) (miller-rabin-test n (- times 1)))
            (else #f)))


(miller-rabin-test 561 50)
; => #f

(miller-rabin-test 2 50)
; => #t

(miller-rabin-test 101 50)
; => #t

(miller-rabin-test 1009 50)
; => #t
